...
class Queue(Extension):
    ...
    async def waiting_queue(self, ctx: InteractionContext):
        ...
        async for playerInDb in VerificationQueue.find(VerificationQueue.status == "New"):
            channel: ThreadChannel =  await self.bot.fetch_channel(1017205079339716688)
    ...
    async def check_queue(self):
        ...
        async for playerWaiting in VerificationQueue.find(VerificationQueue.status == "New"):
            channel: ThreadChannel =  await self.bot.fetch_channel(1009940100068692068)
            guild: Guild = await self.bot.fetch_guild(689119429375819951)
            member = await self.bot.fetch_member(712889161446719508, guild.id)
            player_role: Guild = await guild.fetch_role(843899510483976233)
            waiting_role: Guild = await guild.fetch_role(1001837608030195882)
            ufc_role: Guild = await guild.fetch_role(1009857946458533990)
